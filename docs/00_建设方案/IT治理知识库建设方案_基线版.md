...
> 边界：本方案聚焦“IT治理知识库”。“评审专家知识库”独立建设，通过 Agent 编排调用形成联动。
...
主要内容：流程、制度、风险、控制、岗位、系统、表单、控制证据、OA流程、操作手册。

主要回答：怎么办、谁负责、依据什么、风险是什么、控制点是什么、需要留存什么证据。

### 3.2 评审专家知识库

定位：经验性知识库。

主要内容：可行性分析会纪要、信委会纪要、采购论证会纪要、评估团意见、项目后评估报告、领导历史关注点、项目立项报告修改痕迹、典型项目评审问题清单。

主要回答：应该审什么、怎么判断、可能问什么、量化指标怎么设、哪些问题历史上被关注过、某类项目的评审风险点是什么。

### 3.3 两库联动方式

两个库不合并，通过 Agent 建立联系。
...
KB-REVIEW-EXPERT：评审专家知识库
```

IT治理知识库内部可按标签区分治理域，不建议拆成过多小库。
...
```text
涉及流程、制度、风险、控制、表单、岗位、系统 → 调用 IT治理知识库
涉及项目评审、领导关注、量化指标、可行性分析、采购论证 → 调用 评审专家知识库
同时涉及流程合规和项目评审 → 同时调用两个知识库
```
...
3. 流程变更影响分析：如果项目验收前增加后评估指标确认，会影响哪些流程？
4. 自动画图：画一下信息系统建设项目立项到验收的流程。
5. 与评审专家知识库联动：SCRM客户洞察平台立项报告怎么审？
...
4. OA流程是否可导出节点和表单字段；
5. 制度文件是否有正式版本号和条款编号；
6. 评审专家知识库是否使用同一套 GitHub 仓库管理；
7. 是否需要建立人工审批后的知识库发布流程。
